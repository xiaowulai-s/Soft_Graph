 ...
 *
 *   - `reuse-all`：记录 0 条 ⇒ 两次扫描之间该卷没有任何文件变更 ⇒ 缓存完全有效。✅ 可用（已启用）
 *   - `partial`：只重新处理「名字命中的项」—— 但**新增文件**也在记录里，
 *     没有路径就无法判断它是否落在规则根目录内，会漏检。❌ **已实测排除，不再启用**
 *   - `fallback`：变更记录太多（抖动剧烈的机器），退回签名比对。
 *
 * ── `partial` 的排除依据（v3.0.0 · 复测后定稿）─────────────────
 * 唯一能让 `partial` 安全的路径是「把父文件 ID 反查成目录路径」。
 * `tests/diag-usn-idlookup.ts` 给出了实测数字：
 *
 *   能力：`fsutil file queryFileNameById <卷> <fileid>` —— **无需提权即可用**，
 *         实测 3/3 命中并返回完整路径（G: 为 NTFS，ID 形如 0x…000f7c5f）。
 *   成本：单次 **158ms**（152~163ms，n=3；进程启动占绝大部分）。
 *   批量：**无**。该命令一次只接受一个 fileid，因此成本随变更条数严格线性。
 *
 * 按每轮 200 条变更（抖动目录的典型量级）外推 = **31.6s**，
 * 是签名遍历基线（v2.0.0 实测 GC-12 约 10.0s）的 **316%** ——
 * 也就是「为了省一次签名遍历，先付三次签名遍历的代价」。
 * 结论：明确放弃，第 2 级只保留 `reuse-all`，
 * 增量继续以「卷哨兵 → USN 零变更复用 → 目录签名」三级降级方案运行。
 *
 * 要重新打开这个结论，前提是出现 **批量反查** 通道（一条命令解析多个 ID），
 * 光靠单条调用在数量级上不可能翻盘。
 */
